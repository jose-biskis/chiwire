...
Usage:
  eldenese-allow list
  eldenese-allow add CIDR [CIDR...]
  eldenese-allow rm CIDR [CIDR...]
  eldenese-allow set CIDR [CIDR...]
USAGE
}
...
case "$cmd" in
  ...
  add)
    ...
    eldenese_write_allow_list $current $added
  ...
  rm)
    ...
    # shellcheck disable=SC2086
    eldenese_write_allow_list $kept
    eldenese_apply_config
    eldenese_reload
    current_cidrs
    ;;
  set)
    ...
    next=$(normalize_args "$@")
    # shellcheck disable=SC2086
    eldenese_write_allow_list $next
    eldenese_apply_config
    eldenese_reload
    current_cidrs
    ;;
